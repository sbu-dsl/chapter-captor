...
def process_book(peaks_dir, density_dir, gt_dir, output_dir, book_id):
    if os.path.exists(os.path.join(output_dir, book_id + '_alpha_100.pkl')):
        return book_id, 'exists'
    try:
        with open(os.path.join(peaks_dir, book_id + '.pkl'), 'rb') as f:
            peaks, prominences = pickle.load(f)
        ...
        if len(density) < 2:
            print(book_id, 'empty')
            return book_id, 'empty'

        maximum, minimum = max(prominences), min(prominences)
        denom = maximum - minimum
        prominences = [(x - minimum) / denom for x in prominences]
...
if __name__ == "__main__":
    ...
    peaks_dir = 'window_size_' + str(window_size) + '/test_books_peaks_proms/'
    density_dir = 'window_size_' + str(window_size) + '/test_books_densities/'
    gt_dir = 'test_gt_sentences/'
    output_dir = 'window_size_' + str(window_size) + '/dp/'
    
    if not os.path.exists(peaks_dir):
        print('Invalid peaks dir')
        exit()
    ...
    func = partial(process_book, peaks_dir, density_dir, gt_dir, output_dir)
